...
if (String(Request("MM_insert")) == "form1") {
  ...
  var MM_editTable  = "duanqihunqing";
  var MM_editRedirectUrl = "ahq.asp";
...
%>
<%
var dhq = Server.CreateObject("ADODB.Recordset");
dhq.ActiveConnection = MM_connbbs_STRING;
dhq.Source = "SELECT * FROM duanqihunqing";
dhq.CursorType = 0;
dhq.CursorLocation = 2;
dhq.LockType = 1;
dhq.Open();
var dhq_numRows = 0;
%>
  ...
  <table width="70%" height="243" border="1" cellpadding="1" cellspacing="0" bordercolor="#000000">
    <tr>
      <td><table width="101%" height="239" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
        ...
        <tr>
          <td><input name="hiddenUsername" type="hidden" id="hiddenUsername" value="<%=(dhq.Fields.Item("Username").Value)%>" />
            <input name="hiddenIP" type="hidden" id="hiddenIP" value="<%=(dhq.Fields.Item("IP").Value)%>" /><input type='hidden' name='Time' id='Time'/></td>
          <td><label>
