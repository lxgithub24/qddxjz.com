...
rsfatie.Source = "SELECT ArticleId, Topic, Content, PostTime, Username, IP FROM fatie WHERE ArticleId = "+ rsfatie__MMColParam.replace(/'/g, "''") + "";
...
rsReply.Source = "SELECT * FROM djj WHERE ArticleId = "+ rsReply__MMColParam.replace(/'/g, "''") + "";
...
<table width="100%" height="192" border="1" cellpadding="1" cellspacing="1" bordercolor="#0C5E9E">
  ...
  <tr>
    ...
    头像</td>
    <td width="70%"><a href="replydjj.asp?ArticleId=<%= rsfatie.Fields.Item("ArticleId").Value %>">回复帖子</a></td>
